// CS405SQLInjectionColinKwasnik.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    if (where_pos != std::string::npos)
    {
        ...
        // Regex to match "or <literal> = <same literal>"
        std::regex inj_pattern(R"(or\s+(['\"]?)([a-zA-Z0-9]+)\1\s*=\s*\1\2\1)");

        if (std::regex_search(where_clause, inj_pattern))
        {
